...
import { getDocPage, getAvailableDocSlugs, localizeDocPage } from '@/lib/docsData';
...
export default function DocPage() {
  ...
  const rawDoc = slug ? getDocPage(slug) : null;
  const doc = rawDoc ? localizeDocPage(rawDoc, language) : null;
      ...
      <Layout>
        <section className="border-b border-black bg-white px-4 py-16 text-center sm:px-6 md:px-8 lg:py-24">
          <div className="mx-auto max-w-4xl space-y-8">
            ...
            >
              {t(language, 'docs.notFound')}
            </h1>
            <p className="text-base font-serif text-slate-700 sm:text-lg">{t(language, 'docs.notFoundBody')}</p>
            <Link href="/docs" className="inline-block border-2 border-black bg-primary px-6 py-3 font-sans text-xs font-semibold uppercase tracking-wider text-white transition-colors hover:opacity-90 sm:px-8">
              {t(language, 'docs.backToDocs')}
            </Link>
          </div>
        </section>
      </Layout>
    ...
    <Layout>
      <section className="border-b border-black bg-white px-4 py-12 sm:px-6 md:px-8 lg:py-24">
        <div className="mx-auto max-w-4xl space-y-6 sm:space-y-8">
          <Link href="/docs" className="inline-flex items-center gap-2 text-xs font-sans font-semibold uppercase tracking-wider text-primary hover:underline sm:text-sm">
            <ChevronLeft size={16} />
            {t(language, 'docs.backToDocs')}
          </Link>
          ...
          <div className="flex flex-col gap-2 text-xs font-mono text-slate-600 sm:text-sm">
            <span>
              {t(language, 'docs.area')}: {doc.category === 'research' ? t(language, 'docs.research') : t(language, 'docs.design')}
              {doc.topic ? ` • ${doc.topic}` : ''}
            </span>
            {doc.date && <span>{t(language, 'docs.date')}: {doc.date}</span>}
            {doc.scope && <span className="break-words">{t(language, 'docs.scope')}: {doc.scope}</span>}
          </div>
        </div>
      </section>
        ...
        <section className="border-t border-black bg-white px-4 py-12 sm:px-6 md:px-8 lg:py-24">
          <div className="mx-auto max-w-4xl space-y-8">
            <div className="space-y-2">
              ...
              <h2 className="text-2xl font-serif font-normal sm:text-3xl">{t(language, 'docs.relatedDocs')}</h2>
            </div>

            <div className="grid grid-cols-1 gap-px bg-black p-px md:grid-cols-2">
              {doc.relatedDocs.map((relatedDoc) => (
                <Link key={relatedDoc.slug} href={`/docs/${relatedDoc.slug}`} className="block space-y-3 border-2 border-black bg-background p-5 transition-colors hover:bg-slate-50 sm:p-6">
        ...
        <section className="border-t border-black bg-background px-4 py-12 sm:px-6 md:px-8 lg:py-24">
          <div className="mx-auto max-w-4xl space-y-8">
            <div className="space-y-2">
              ...
              <h2 className="text-2xl font-serif font-normal sm:text-3xl">{t(language, 'docs.wikilinks')}</h2>
            </div>
            ...
            <div className="space-y-4 border-2 border-black bg-white p-5 sm:p-8">
              <p className="mb-6 text-xs font-mono text-slate-600 sm:text-sm">
                {t(language, 'docs.wikilinksInfo')} {wikilinks.length} {t(language, 'docs.wikilinksRef')}
              </p>
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                          {t(language, 'docs.displayedAs')}: <span className="font-serif">{link.displayText}</span>
                        </p>
                      </div>
                      {exists ? (
                        <Link href={`/docs/${link.linkText}`} className="text-xs font-sans font-semibold uppercase tracking-wider text-primary hover:underline">
                          {t(language, 'docs.open')}
                        </Link>
                      ) : (
                        <span className="text-xs font-sans font-semibold uppercase tracking-wider text-slate-400">
                          {t(language, 'docs.pending')}
                        </span>
                      )}
                    </div>
                  );
